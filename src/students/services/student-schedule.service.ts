...
import { StudentScheduleDto, DailyScheduleDto, ClassScheduleDto, AcademicHistoryDto, CourseHistoryDto } from '../dto/student-schedule.dto';
...
export class StudentScheduleService {
  ...
  async getStudentAcademicHistory(studentId: string): Promise<AcademicHistoryDto> {
    ...
    for (const enrollment of allEnrollments) {
      ...
      const courseHistory: CourseHistoryDto = {
        ...
        color: this.getTrafficLightColor(enrollment.status)
      };
  ...
  private getTrafficLightColor(status: EnrollmentStatus): 'green' | 'yellow' | 'red' {
    switch (status) {
      case EnrollmentStatus.PASSED:
        return 'green';
      case EnrollmentStatus.ENROLLED:
