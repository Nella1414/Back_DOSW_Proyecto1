...
export class ScheduleValidationService {
  ...
  async validateChangeRequest(
    ...
    try {
      // 1. Validar que el periodo permite cambios
      ...
      // 4. Validar que el estudiante está inscrito en el grupo origen
      ...
      // 5. Validar que los grupos son del mismo curso
  ...
  async validateScheduleConflicts(
    ...
    // Obtener horarios actuales del estudiante (excluyendo el grupo origen)
  ...
  ): Array<{day: string, time1: string, time2: string}> {
    const conflicts: Array<{day: string, time1: string, time2: string}> = [];
